...
### API 3（推奨）: `works`（作品アーカイブ）

| フィールド ID | 種類 | 必須 | 内容 |
| --- | --- | --- | --- |
| `title` | テキストフィールド | ○ | 作品名。例: `古従軍行` |
| `style` | セレクトボックス | ○ | `楷書` `行書` `草書` `かな` `篆刻` `その他` |
| `exhibition` | テキストフィールド | – | 展示した書展名。例: `冬樟展 2024` |
| `author` | テキストフィールド | – | 筆者名 |
| `image` | 画像フィールド | ○ | 作品画像 |
| `caption` | テキストエリア | – | 作品ノート |

> 書展個別ページは、`works` API の `exhibition` フィールドが書展名と部分一致する作品を自動で「展示作品」として表示します。
...
1. microCMS 管理画面 → 対象 API（`news`, `exhibitions`, `works`）→ **API設定** → **Webhook**
...
```
src/
├── components/      # Nav / Footer / Hero / NewsList / WorksGrid 等の再利用コンポーネント
