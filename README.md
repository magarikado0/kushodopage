...
| `/` | トップページ |
| `/about` | 書道部について・入部案内 |
| `/activities` | 活動記録（書展・書道パフォーマンス） |
| `/exhibitions` | `/activities` へのリダイレクト |
...
microCMS 管理画面で以下 **3 つの API** を作成してください。
...
| `description` | テキストエリア | – | トップページに表示する短い広報文。未入力時は本文から自動で抜粋 |
...
| `startDate` | 日時フィールド | ○ | 開始日時。開催年はこの値から自動表示 |
| `endDate` | 日時フィールド | ○ | 終了日時 |
| `venue` | テキストフィールド | ○ | 会場名 |
| `address` | テキストフィールド | – | 住所 |
| `description` | テキストエリア | ○ | 書展の紹介文 |
| `heroImage` | 画像フィールド | – | メインビジュアル |
| `brochureUrl` | テキストフィールド | – | 書展パンフレットのURL |
| `artoirUrl` | テキストフィールド | – | Artoirの作品アーカイブURL |
| `featured` | 真偽値フィールド | – | `true` にするとトップ「次回の書展」に表示 |

### API 3: `performances`（書道パフォーマンス）
...
| --- | --- | --- | --- |
| `year` | 数値フィールド | ○ | 開催年 |
| `type` | セレクトボックス | ○ | `NF書道パフォーマンス` または `新歓書道パフォーマンス` |
| `url` | テキストフィールド | ○ | 写真・動画などの外部リンク |
| `description` | テキストエリア | – | 短い紹介文 |
...
4. 各 API（`news`, `exhibitions`, `performances`）について同じ URL を設定
